...
getmetatable(timer.proxy).__gc = function()
	print("timer collected.")
end
...
getmetatable(channel.proxy).__gc = function()
	print("channel collected.")
end
channel:addEventListener(Event.COMPLETE, function()
	print("channel complete.")
end)

-- UrlLoader test
local urlloader = UrlLoader.new("https://www.google.com")

urlloader.proxy = newproxy(true)
getmetatable(urlloader.proxy).__gc = function()
	print("urlloader collected.")
end
