...
v1.0.5 - 01.11.2012
Some optimisation with local variables
...
-- optimisation
local _newTimer = os.timer
local _newSprite = Sprite.new
local _newEvent = Event.new
...
local function dispatchEvent(dispatcher, name)
	if dispatcher:hasEventListener(name) then
		dispatcher:dispatchEvent(_newEvent(name))
	end
end
...
function SceneManager:init(scenes)
	self.scenes = scenes
	self.tweening = false
	self.transitionEventCatcher = _newSprite()
...
function SceneManager:changeScene(scene, duration, transition, ease, options)
	...
	if self.scene1 == nil then
		local scene1 = self.scenes[scene].new(options and options.userData)
		self:addChild(scene1)
		dispatchEvent(self, "transitionBegin")
		dispatchEvent(scene1, "enterBegin")
		dispatchEvent(self, "transitionEnd")
		dispatchEvent(scene1, "enterEnd")
	...
	local scene2 = self.scenes[scene].new(options and options.userData)
...
function SceneManager:onTransitionBegin()
	if self.eventFilter then
		local transitionEventCatcher = self.transitionEventCatcher
		local filterTransitionEvents = self.filterTransitionEvents

		stage:addChild(transitionEventCatcher)
		for i,event in ipairs(self.eventFilter) do
			transitionEventCatcher:addEventListener(event, filterTransitionEvents, self)
		end
	end
end
...
function SceneManager:onTransitionEnd()
	if self.eventFilter then
        	...
        	for i,event in ipairs(self.eventFilter) do
			transitionEventCatcher:removeEventListener(event, filterTransitionEvents, self)
		end
		transitionEventCatcher:removeFromParent()
	end
end
...
function SceneManager:onEnterFrame(event)
	...
	local scene1 = self.scene1
	local scene2 = self.scene2

	if time == 0 then
		self:onTransitionBegin()
		scene2:setVisible(true)
		dispatchEvent(self, "transitionBegin")
		dispatchEvent(scene1, "exitBegin")
		dispatchEvent(scene2, "enterBegin")
	end
		
	local timer = _newTimer()
	local deltaTime = timer - self.currentTimer
	self.currentTimer = timer

	local t = (duration == 0) and 1 or (time / duration)

	self.transition(scene1, scene2, self.ease(t), t)

	if time == duration then
		dispatchEvent(self, "transitionEnd")
		dispatchEvent(scene1, "exitEnd")
		dispatchEvent(scene2, "enterEnd")
		self:onTransitionEnd()

		self:removeChild(scene1)
		self.scene1 = scene2
		self.scene2 = nil
		self.tweening = false
