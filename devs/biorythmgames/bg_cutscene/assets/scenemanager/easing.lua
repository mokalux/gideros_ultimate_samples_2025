...
easing.inElastic = function(ratio)
	...
	ratio = ratio - 1
	return -(elasticA * pow(2, 10 * ratio) * sin((ratio - elasticS) * (2 * pi) / elasticP));
end
easing.outElastic = function(ratio)
	if ratio == 0 or ratio == 1 then
		return ratio
	end
	return elasticA * pow(2, -10 * ratio) *  sin((ratio - elasticS) * (2 * pi) / elasticP) + 1;
end
easing.inOutElastic = function(ratio)
	...
	ratio = ratio*2-1
	if ratio < 0 then
		return -0.5 * (elasticA * pow(2, 10 * ratio) * sin((ratio - elasticS*1.5) * (2 * pi) /(elasticP*1.5)));
	end
	return 0.5 * elasticA * pow(2, -10 * ratio) * sin((ratio - elasticS*1.5) * (2 * pi) / (elasticP*1.5)) + 1;
end
