...
function StartScene:onStartTouched(event)
	local loadingPopup=LoadingPopup.new(
		{
			autoRunFunc=function()
				self:doSomeLongLoadingThing()
			end,
			minimumShowTime=2
		}
	)
	loadingPopup:addEventListener("complete", function()
		mySceneManager:changeScene("ForestScene", 0.5, SceneManager.moveFromRight)
	end)
end

function StartScene:init()
	local text=TextField.new(TTFont.new("fonts/KenVector Future.ttf", 40), "Touch to Start")
