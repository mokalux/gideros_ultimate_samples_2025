...
-- update to use the new operators  (sinistersoft)
...
function CEmitter:init(xPos, yPos, direction, parentGroup)
	...
	self.emitter = {
		...
		direction = ^<direction, -- direction (angle) of emitter
...
function CEmitter:getDirection()
	return ^>self.emitter.direction	-- get emitter direction
end
...
function CEmitter:setDirection(direction)
	self.emitter.direction = ^<direction -- set emitter direction
end
				...
				--		end
					...
					if particles.colorMorphIn and particlePoint._ColorMorphIn then
						if timePassed <= particlePoint.targetColorTimeIn then
							...
							particlePoint.cr += (particlePoint.trIn - currentR) / tStep
							particlePoint.cg += (particlePoint.tgIn - currentG) / tStep
							particlePoint.cb += (particlePoint.tbIn - currentB) / tStep
						end
					else
					...
					if particles.colorMorphOut and particlePoint._ColorMorphOut then
						...
						if rTime > 0 then
							if timePassed >= particles.particlesMaxLife - particlePoint.targetColorTimeOut then
								particlePoint.cr += (particlePoint.trOut - particlePoint.cr) / rTime
								particlePoint.cg += (particlePoint.tgOut - particlePoint.cg) / rTime
								particlePoint.cb += (particlePoint.tbOut - particlePoint.cb) / rTime
								currentR = particlePoint.cr
								currentG = particlePoint.cg
								currentB = particlePoint.cb
							end
					...
					if particles.speedMorphIn and particlePoint._SpeedMorphIn then
						if timePassed <= particlePoint.targetSpeedTimeIn then
							particlePoint.currentSpeed += (particlePoint.targetSpeedIn - speed) / ((particlePoint.targetSpeedTimeIn) / event.deltaTime)
					...
					if particles.speedMorphOut and particlePoint._SpeedMorphOut then
						...
						if rTime > 0 then
							if timePassed >= (particles.particlesMaxLife - particlePoint.targetSpeedTimeOut) then
								particlePoint.currentSpeed += (particlePoint.targetSpeedOut - speed) / rTime
								speed = particlePoint.currentSpeed
							end
						end
					else
					...
					if particles.directionMorphIn and particlePoint._directionMorphIn then
						if timePassed <= particlePoint.targetDirectionTimeIn then
							particlePoint.currentDirection += (particlePoint.targetDirectionIn - direction) / ((particlePoint.targetDirectionTimeIn) / event.deltaTime)
					...
					if particles.directionMorphOut and particlePoint._directionMorphOut then
						...
						if rTime > 0 then
							if timePassed >= (particles.particlesMaxLife - particlePoint.targetDirectionTimeOut) then
								direction = particlePoint.currentDirection
								particlePoint.currentDirection += (particlePoint.targetDirectionOut - direction) / rTime
								direction = (particlePoint.currentDirection)
							end
						end
					else
					...
					if particles.sizeMorphIn and particlePoint._SizeMorphInEnabled then
						if timePassed <= particlePoint.targetSizeTimeIn then
							particlePoint.currentSize += (particlePoint.targetSizeIn - cSize) / (particlePoint.targetSizeTimeIn / event.deltaTime)
					...
					if particles.sizeMorphOut and particlePoint._SizeMorphOutEnabled then
						...
						if rTime > 0 then
							if timePassed >= (particles.particlesMaxLife - particlePoint.targetSizeTimeOut) then
								particlePoint.currentSize += (particlePoint.targetSizeOut - cSize) / rTime
								cSize = particlePoint.currentSize
							end
						end
					else
					...
					if particles.alphaMorphIn and particlePoint._AlphaMorphInEnabled then
						if timePassed <= particlePoint.targetAlphaTimeIn then
							--     particlePoint.ca += (particlePoint.targetAlphaIn - alpha) / ((particlePoint.targetAlphaTimeIn) / event.deltaTime)
							-- FIX by Paolo Manna
							particlePoint.ca += (particlePoint.targetAlphaIn - particlePoint.a) / ((particlePoint.targetAlphaTimeIn) / event.deltaTime)
							alpha = particlePoint.ca
						end
					else
						alpha = particlePoint.ca
					...
					if particles.alphaMorphOut and particlePoint._AlphaMorphOutEnabled then
						...
						if rTime > 0 then
							if timePassed >= (particles.particlesMaxLife - particlePoint.targetAlphaTimeOut) then
								particlePoint.ca += (particlePoint.targetAlphaOut - alpha) / rTime
								--        particlePoint.ca += (particlePoint.targetAlphaOut - alpha) / rTime
								alpha = particlePoint.ca
							end
						end
					...
					if particles.rotation then
						-- rotate always
						particlePoint.rotation += particlePoint.rotationVelocity * event.deltaTime
						particles.particlesGroup:setParticleAngle(particlePoint.sprite,-particlePoint.rotation)
					else
						local newAngle = ^>(self.emitter.direction + particlePoint.direction)
					...
					-- GRAVITY ---------------------------------------------
					--------------------------------------------------------
					particlePoint.gravityForceX += particles.gravityX * event.deltaTime
					particlePoint.gravityForceY += particles.gravityY * event.deltaTime
					--------------------------------------------------------
					-- POSITION --------------------------------------------
					--------------------------------------------------------
					particlePoint.xPos += (particlePoint.gravityForceX + (qCos(self.emitter.direction + direction) * speed)) * event.deltaTime
					particlePoint.yPos += (particlePoint.gravityForceY + (qSin(self.emitter.direction + direction) * speed)) * event.deltaTime
					--------------------------------------------------------
					-- WRITE SPRITE ----------------------------------------
					--------------------------------------------------------
					...
					if (particlePoint.loopCounter <= particles.loopCounter) then
						if self.emitter.start then
							...
							if particles.randomDirection then
								particlePoint.direction = ^<qRandom(particles._directionMin, particles._directionMax)
								particlePoint.currentDirection = particlePoint.direction
							else
								particlePoint.direction = ^<particles._directionMin
								particlePoint.currentDirection = particlePoint.direction
							end
							-------------------------------------------------------------------------
							-- Displacement ---------------------------------------------------------
							...
							if particles.directionMorphIn then
								...
								if particles.randomDirectionMorphIn then
									particlePoint.targetDirectionIn = ^<qRandom(particles._targetDirectionMorphInMin, particles._targetDirectionMorphInMax) + particlePoint.direction
									...
									particlePoint.targetDirectionIn = ^<particles._targetDirectionMorphInMin + particlePoint.direction -- ***
							...
							if particles.directionMorphOut then
								...
								if particles.randomDirectionMorphOut then
									particlePoint.targetDirectionOut = ^<qRandom(particles._targetDirectionMorphOutMin, particles._targetDirectionMorphOutMax) + particlePoint.direction
									...
									particlePoint.targetDirectionOut = ^<particles._targetDirectionMorphOutMin + particlePoint.direction
							...
							-------------------------------------------------------------------------
								...
								particles.particlesGroup:setParticlePosition(particlePoint.sprite,particlePoint.moveXPos + particlePoint.xPos + particlePoint.xDisplacement + particles.xOffset, particlePoint.moveYPos + particlePoint.yPos + particlePoint.yDisplacement + particles.yOffset)
								particles.particlesGroup:setParticleAngle(particlePoint.sprite,-^>(self.emitter.direction + particlePoint.direction))
...
function CParticles:setDirectionMorphIn(targetAngle, targetTime, ...)
	...
	--particles._directionMin += targetAngle
