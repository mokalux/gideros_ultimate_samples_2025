...
function Progressbar:init(config)
	...
	if config then -- copying configuration
	...
	if self.conf.showValue then
		self.text = TextField.new(
			self.conf.textFont, self.conf.textBefore..self.value..self.conf.textAfter
		)
		self:positionText()
...
function Progressbar:positionText()
	...
	self.text:setPosition(
		math.floor((self:getWidth() - w)/2) - self.text.xOffset,
		math.floor((self:getHeight() - h)/2) - self.text.yOffset
	)
end
...
function Progressbar:drawBackground()
	...
	if self.conf.radius == 0 then
		...
		self:quadraticCurveTo(
			self.bg, 0, self.conf.height - self.conf.radius,
			0, self.conf.height, 
			self.conf.radius, self.conf.height
		)
		self.bg:lineTo(self.conf.width - self.conf.radius, self.conf.height)
		self:quadraticCurveTo(
			self.bg, self.conf.width - self.conf.radius, self.conf.height,
...
function Progressbar:drawBar()
	if self.value > 0 then
		local width =
			((self.value-self.conf.minValue)/(self.conf.maxValue-self.conf.minValue))*self.conf.width
		self.bar:clear()
