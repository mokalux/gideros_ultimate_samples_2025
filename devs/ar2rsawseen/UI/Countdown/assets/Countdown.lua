...
-- math.mod deprecated, use math.fmod
...
function Countdown:init(config)
	self.timer = Timer.new(1000) -- 1000, timer interval = 1000ms = 1 sec
...
function Countdown:normalize()
	self.conf.min = self.conf.min + floor(self.conf.sec/60);
	self.conf.sec = self.conf.sec % 60;
	
	self.conf.hour = self.conf.hour + floor(self.conf.min/60);
	self.conf.min = self.conf.min % 60;
	
	self.conf.day = self.conf.day + floor(self.conf.hour/24);
	...
	if(self.conf.day > leftover) then
		...
		self.conf.week = self.conf.week + floor(self.conf.day/7)
		self.conf.day = self.conf.day % 7
	end
	
	self.conf.year = self.conf.year + floor(self.conf.month/12)
...
function Countdown:recalc()
	...
	if(not self.abr.w.used) then
		...
		self.abr.w.value = self.abr.w.value + floor(self.abr.d.value/7);
