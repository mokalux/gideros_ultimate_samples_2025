...
distanceJoint = gideros.class(Sprite)

--on scene initialization
function distanceJoint:init()
	...
	local bottom = self:wall(400,480,10,10)
	--some spring bounds
	self:springBound(369,440,1,150)
	self:springBound(431,440,1,150)
	
	self.spring = Bitmap.new(Texture.new("spring.png", true))
	self.spring:setAnchorPoint(0.5, 1)
	self.spring:setPosition(400, 480)
	self.spring.original = self.spring:getHeight()
	self:addChild(self.spring)
	
	--create ball
	self.ball1 = self:springTop(400, 380)
	self.ball2 = self:ball(400, 100)
	local jointDef = b2.createDistanceJointDef(self.ball1.body, bottom.body, 400, 380, 400, 480)
	...
	distanceJoint:setLength(100)
	distanceJoint:setDampingRatio(0.1)
	distanceJoint:setFrequency(3.5)
...
function distanceJoint:wall(x, y, width, height)
...
function distanceJoint:springBound(x, y, width, height)
	local wall = Shape.new()
	--define wall shape
	wall:beginPath()
...
-- as example - ball
function distanceJoint:ball(x, y)
...
--	local fixture = body:createFixture{shape = circle, density = 1.0, 
	local fixture = body:createFixture{shape = circle, density = 8.0, 
--	friction = 0.1, restitution = 0.8}
...
function distanceJoint:springTop(x, y)
	--create ball bitmap object from ball graphic
	local ball = Bitmap.new(Texture.new("./springTop.png"))
	--reference center of the ball for positioning
	ball:setAnchorPoint(0.5,0.5)
	...
	poly:setAsBox(30, 5)
--	local fixture = body:createFixture{shape = poly, density = 1.0, 
	local fixture = body:createFixture{shape = poly, density = 16.0, 
--	friction = 0.1, restitution = 0.8}
	friction = 0.1, restitution = 1}
...
--running the world
function distanceJoint:onEnterFrame() 
	...
	--update spring
	local distance = self.spring:getY() - self.ball1:getY()
...
function distanceJoint:onExitBegin()
  self:removeEventListener(Event.ENTER_FRAME, self.onEnterFrame, self)
end

stage:addChild(distanceJoint.new())