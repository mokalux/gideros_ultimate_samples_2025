...
frictionJoint = gideros.class(Sprite)

--on scene initialization
function frictionJoint:init()
	local text = TextField.new(TTFont.new("tahoma.ttf", 20), "Friction Joint example")
...
function frictionJoint:wall(x, y, width, height)
...
-- as example - ball
function frictionJoint:ball(x, y)
...
--running the world
function frictionJoint:onEnterFrame() 
...
function frictionJoint:onExitBegin()
