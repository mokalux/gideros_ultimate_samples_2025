...
wheelJoint = gideros.class(Sprite)

--on scene initialization
function wheelJoint:init()
	local text = TextField.new(TTFont.new("tahoma.ttf", 20), "Wheel Joint example")
	text:setPosition(math.floor((application:getContentWidth()-text:getWidth())/2), 100)
	self:addChild(text)
	--create world instance
	self.world = b2.World.new(0, 48, true) -- 10
...
--	local jointDef = b2.createWheelJointDef(self.ball1.body, ground, 350, 200, 1, 1)
	local jointDef = b2.createWheelJointDef(self.ball1.body, ground, 350, 200, 0, 1)
...
function wheelJoint:wall(x, y, width, height)
...
--running the world
function wheelJoint:onEnterFrame() 
...
function wheelJoint:onExitBegin()
