...
revoluteJoint = gideros.class(Sprite)

--on scene initialization
function revoluteJoint:init()
	local text = TextField.new(TTFont.new("tahoma.ttf", 20), "Revolute Joint example")
...
--	ground:setPosition(300, 480)
	ground:setPosition(350, 300)
	
	--create ball
	self.ball1 = self:ball(350, 100)
	self.ball2 = self:ball(300, 300)
	local jointDef = b2.createRevoluteJointDef(self.ball2.body, ground, 300, 300)
...
function revoluteJoint:wall(x, y, width, height)
...
-- as example - ball
function revoluteJoint:ball(x, y)
...
--running the world
function revoluteJoint:onEnterFrame() 
...
function revoluteJoint:onExitBegin()
