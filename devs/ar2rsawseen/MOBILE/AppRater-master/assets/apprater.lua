...
do
	...
	function table.save(  tbl,filename )
		...
		elseif filename == true or filename == 1 then
			...
			-- write table to file
			-- use io.open here rather than io.output, since in windows
			-- when clicking on a file opened with io.output will create an error
		else
...
function AppRater:rate()
	local alertDialog = AlertDialog.new(
		self.conf.rateTitle,
		self.conf.rateText,
		self.conf.cancelButton, self.conf.rateButton, self.conf.remindButton
	)
	...
	local function onComplete(event)
		if event.buttonIndex == 1 then
			...
			if osName == "Android" and self.conf.androidRate ~= "" then
				print(self.conf.androidRate)
				-- uncomment following line when app is released on store
--				application:openUrl(self.conf.androidRate)
			elseif osName == "iOS" and self.conf.iosRate ~= "" then
				print(self.conf.iosRate)
				-- uncomment following line when app is released on store
