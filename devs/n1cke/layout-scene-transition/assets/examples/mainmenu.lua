local function startswith(s, piece)
  return string.sub(s, 1, string.len(piece)) == piece
end

local scenes = Layout.newResources{ -- copied some examples from examples folder
	path = "|R|scenes",
	namemod = function(name, path, base, ext, i) return base end
}

local layout

local backbutton = Layout.new{
	texture = backbuttonTexture,
	ancX = 1, ancY = 0,
	relW = 0.1, relH = 0.1,
	limW = 1, limH = 1,
	anPress = Layout.newAnimation(14, 7, 0.04),
	anHover = Layout.newAnimation(14, 7, 0.02),
	anAdd = Layout.newAnimation(),
	anRemove = Layout.newAnimation(),
	onPress = function(self)
		for _,child in pairs(stage.__children) do
			child:removeFromParent()
		end
		stage:addChild(layout1)
		stage:addChild(layout2)
		Layout.select(layout1)
	end
}

backbutton:addEventListener(Event.ENTER_FRAME, function()
	if backbutton:getParent() == stage then
		stage:addChild(backbutton) -- super sprite, always on top
	end
end)

local Button = Layout:with{
	text = "BUTTON",
	textColor = 0xFFFFFF,
	bgrA = 1.0,
	
	init = function(self, p)
		self.textfield = TextField.new(font, self.text, "Pq|")
		self.textfield:setTextColor(self.textColor)
		self:addChild(self.textfield)
	end,
	
	upd = function(self, p)
		if p.text then self.textfield:setText(p.text) end
	end,
	
	anPress = Layout.newAnimation(14, 7, 0.04),
	anHover = Layout.newAnimation(14, 7, 0.02),
	
	selector = Layout.new{bgrC = 0x0088FF, bgrA = 0.25},
	
	onPress = function(self)
		stage:addChild(backbutton)
		Layout.select(backbutton)
		
		if startswith(self.text,"scene") then
--			print(self.text)
			scenes[self.text]()
			dataSaver.saveValue("sets", sets)
		else
			examples[self.text]()
		end
		layout1:removeFromParent()
		layout2:removeFromParent()
	end
}

local database1 = {}
for i = 1, 9 do database1[i] = {text = "scene"..i} end

local database = {}
for i = 1, #examples do database[i] = {text = examples[-i]} end

layout1 = Layout.new{
	template = Button, database = database1, scroll = true,
	anAdd = Layout.newAnimation(),
	--~ anRemove = Layout.newAnimation(),
	
	--~ anAdd = {frames = 60, mark = 0, rotation = 1, anchorX = 1},
	--~ anRemove = {frames = 60, mark = -1, scaleX = math.log, alpha = math.sqrt},
	
	cellAbsH = 150,cellAbsW = 150,
	cellBrdW = 10, cellBrdH = 10,
	cols = 3, rows = 2,
	absX = 60 , absY = 125,
}
layout2 = Layout.new{
	template = Button, database = database, scroll = true,
	anAdd = Layout.newAnimation(),
	--~ anRemove = Layout.newAnimation(),
	
	--~ anAdd = {frames = 60, mark = 0, rotation = 1, anchorX = 1},
	--~ anRemove = {frames = 60, mark = -1, scaleX = math.log, alpha = math.sqrt},
	
	cellAbsH = 50,cellAbsW = 150,
	cellBrdW = 5, cellBrdH = 5,
	cols = 3, rows = 5,
	absX = 70 , absY = 625,
}

stage:addChild(layout1)
stage:addChild(layout2)

Layout.select(layout1)
