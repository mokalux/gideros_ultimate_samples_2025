...
function SceneManager:changeScene(scene, duration, transition, ease, options)
	...
	---------------------------------------------------------------------------------------------------------
	-- for more detail see:
	-- http://giderosmobile.com/forum/discussion/2859/scenemanager-howto-transition-only-after-loading
	---------------------------------------------------------------------------------------------------------
	
	Timer.delayedCall(1, function()	-- Fix version 1.1
		self.time = 0
		self.currentTimer = os.timer()
		self.tweening = true	-- Fix version 1.1
	end)
