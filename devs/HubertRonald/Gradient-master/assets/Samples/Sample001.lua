...
--	local datos = load("|D|data")
	local datos = load("gradients/gradients_demo")
...
--[[
			...
			texture = {"Sources/Images/Landscapes/pexels-photo-89432.png",true},	-- if {} you don't want this option
															-- or ike always	{"image.png", false, {transparentColor = 0xff00ff}}
															--					{"image.png", true, {wrap = Texture.REPEAT}}
															-- Pixel Data		{nil,300,400;, false, {extend=false}}
			anchorTexture={.2,.5},
			scaleTexture = {0.01,0.01},		-- sX,sY calculate for you scale perfect when deform is false
										-- and take larger axis scale viz {.6,0} equal to {.6,.6} when deform is false
			deform = true,				-- if you don't need animation like to jelly
			colorOn = true,
