...
-- The `grid` is an implicit graph which represents the 2D world map layout
-- on which the `pathfinder` object will run
-- During a search, the `pathfinder` object needs to save some critical values
-- These values are cached within each `node` object, and the whole set of nodes
-- are tied inside the `grid` object itself

-- Local references
local Utils = require "GJumper/core/utils"
local Assert = require "GJumper/core/assert"
local Node = require "GJumper/core/node"
...
--- The `Grid` class.<br/>
-- This class is callable, therefore,
-- <code>Grid(...)</code> _acts as a shortcut to <code>Grid:new(...)</code>
-- @type Grid
local Grid = {}
Grid.__index = Grid
...
--		return walkable(nodeValue) and hasEnoughClearance
		return walkable(nodeValue, x, y) and hasEnoughClearance -- GH fix
	end
	...
	for i = 1,#diagonalOffsets do
		...
		if n and self:isWalkableAt(n._x, n._y, walkable, clearance) then
			if tunnel then
				...
				if ((n1 and n2) and not self:isWalkableAt(n1._x, n1._y, walkable, clearance) and not
					self:isWalkableAt(n2._x, n2._y, walkable, clearance)) then
						skipThisNode = true
				end
