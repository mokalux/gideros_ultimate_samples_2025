...
-- Dependencies
local Utils     = require "GJumper/core/utils"
local Assert    = require "GJumper/core/assert"
--local Heap      = require "GJumper/core/bheap"
local Heuristic = require "GJumper/core/heuristics"
local Grid      = require "GJumper/grid"
--local Path      = require "GJumper/core/path"
...
local Finders = {
	["ASTAR"]     = require "GJumper/search/astar",
	["DIJKSTRA"]  = require "GJumper/search/dijkstra",
	["THETASTAR"] = require "GJumper/search/thetastar",
	["BFS"]       = require "GJumper/search/bfs",
	["DFS"]       = require "GJumper/search/dfs",
	["JPS"]       = require "GJumper/search/jps"
}
...
function Pathfinder:annotateGrid(allDirections)
	...
	if allDirections then
		self:annotateGridOneDirection(
			upleft, self._grid._min_x, self._grid._max_x, 1, self._grid._min_y, self._grid._max_y, 1, -1, -1)
		self:annotateGridOneDirection(
			upright, self._grid._max_x, self._grid._min_x, -1, self._grid._min_y, self._grid._max_y, 1, 1, -1)
		self:annotateGridOneDirection(
			downleft, self._grid._min_x, self._grid._max_x, 1, self._grid._max_y, self._grid._min_y, -1, -1, 1)
	end
	self:annotateGridOneDirection(
		downright, self._grid._max_x, self._grid._min_x, -1, self._grid._max_y, self._grid._min_y, -1, 1, 1)

	-- assign the clearance to the grid
	for y = self._grid._min_y, self._grid._max_y do
		for x = self._grid._min_x, self._grid._max_x do
			local node = self._grid:getNodeAt(x, y)
			node._clearance[self._walkable] =
				allDirections and math.min(upleft[y][x], upright[y][x], downleft[y][x], downright[y][x]) or downright[y][x]
		end
	end
...
function Pathfinder:setFinder(finderName)
	if not finderName then
		if not self._finder then finderName = "ASTAR"
		else return
		end
	end
