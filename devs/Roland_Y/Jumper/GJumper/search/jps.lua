...
-- Dependancies
local Heuristics = require "GJumper/core/heuristics"
local Heap = require "GJumper/core/bheap"
...
local function findNeighbours(finder, node, clearance)
	if node._parent then
		...
		local dx = (x-node._parent._x)/max(-(x-node._parent._x)<>(x-node._parent._x),1)
		local dy = (y-node._parent._y)/max(-(y-node._parent._y)<>(y-node._parent._y),1)
...
return finder._grid:getNeighbours(node, finder._walkable, finder._allowDiagonal, finder._tunnel, clearance) end
...
local function jump(finder, node, parent, endNode, clearance)
	...
	if dx ~= 0 and dy ~= 0 then
		...
		if (finder._grid:isWalkableAt(x-dx, y+dy, finder._walkable, clearance) and
			(not finder._grid:isWalkableAt(x-dx, y, finder._walkable, clearance))) or
			(finder._grid:isWalkableAt(x+dx, y-dy, finder._walkable, clearance) and
			(not finder._grid:isWalkableAt(x, y-dy, finder._walkable, clearance))) then
				return node
		end
	else
		...
		if dx ~= 0 then
			if finder._allowDiagonal then
				...
				if (finder._grid:isWalkableAt(x+dx, y+1, finder._walkable, clearance) and
					(not finder._grid:isWalkableAt(x, y+1, finder._walkable, clearance))) or
					(finder._grid:isWalkableAt(x+dx, y-1, finder._walkable, clearance) and
					(not finder._grid:isWalkableAt(x, y-1, finder._walkable, clearance))) then
						return node
				end
			else
				-- : in case diagonal moves are forbidden
				if finder._grid:isWalkableAt(x+1, y, finder._walkable, clearance) or
					finder._grid:isWalkableAt(x-1, y, finder._walkable, clearance) then
						return node
				end
			end
		else
			...
			if finder._allowDiagonal then
				if (finder._grid:isWalkableAt(x+1, y+dy, finder._walkable, clearance) and
					(not finder._grid:isWalkableAt(x+1, y, finder._walkable, clearance))) or
					(finder._grid:isWalkableAt(x-1, y+dy, finder._walkable, clearance) and
					(not finder._grid:isWalkableAt(x-1, y, finder._walkable, clearance))) then
						return node
				end
			else
				-- : in case diagonal moves are forbidden
				if finder._grid:isWalkableAt(x, y+1, finder._walkable, clearance) or
					finder._grid:isWalkableAt(x, y-1, finder._walkable, clearance) then
						return node
				end
			end
		end
	...
	if finder._allowDiagonal then
		if finder._grid:isWalkableAt(x+dx, y, finder._walkable, clearance) or
			finder._grid:isWalkableAt(x, y+dy, finder._walkable, clearance) then
		...
		if jumpNode and not skip then
			...
			if not jumpNode._closed then
				...
				if not jumpNode._opened or newG < jumpNode._g then
					...
					jumpNode._g = newG
					jumpNode._h = jumpNode._h or (finder._heuristic(jumpNode, endNode))
