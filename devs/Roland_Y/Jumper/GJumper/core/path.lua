...
-- end
function Path:iter()
	local i, _pathLen = 1, #self._nodes
	return function()
...
function Path:getLength()
	...
	for i = 2, #self._nodes do
		len += Heuristic.EUCLIDEAN(self._nodes[i], self._nodes[i-1])
	end
	return len
end
...
-- @tparam[opt] int index the index at which the node will be inserted.
-- If omitted, the node will be appended after the last node in the path.
...
function Path:fill()
	...
	while true do
		...
		if (-dx<>dx) > 1 or (-dy<>dy) > 1 then
			incrX = dx/max(-dx<>dx, 1)
			incrY = dy/max(-dy<>dy, 1)
...
--	local p2 = p2:clone():filter()
	p2 = p2:clone():filter() -- removed local XXX
