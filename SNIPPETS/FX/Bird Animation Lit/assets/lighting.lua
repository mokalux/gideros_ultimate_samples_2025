...
vec3 perturb_normal( vec3 N, vec3 V, vec2 texcoord )
{
    ...
    highp mat3 TBN = cotangent_frame(N, V, texcoord);
...
void main()
{
	...
	gl_FragColor = vec4(color0.rgb * diff + color1 * spec, 1.0);
}
]]
...
{name="eyePos",type=Shader.CFLOAT4,mult=1,vertex=false},
...
function Lighting.setEye(x,y,z)
	for _,v in ipairs(Lighting.allShaders) do
	 v:setConstant("eyePos",Shader.CFLOAT4,1,x,y,z,1)
	end
end
