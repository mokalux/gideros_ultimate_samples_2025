...
--pubnub module initiation, called the first time before the functions below
...
--	self.connectionLostHandler = EventHandler.new(self)
	self.connectionLostHandler = Event.new("connectionLost")
	self.send_stack = {}
	...
	self.stackMsg = {} -- stack tempat message yang akan dipublish, EN: stack where messages will be published
	self.updateTime = 500 -- durasi pengiriman, EN: delivery duration
	...
	self.server_pubnub:time({
		callback = function(time)
			if (time==nil) then
				self.myPlayer.id = "1."..math.random(1, 999999999).. -- (1, 99999999999)
					"e+"..math.random(1,999)..
...
--		Log.d(LOG_TAG, "timeout!", self.counter_time_out_connection)
		print(LOG_TAG, "timeout!", self.counter_time_out_connection)
...
--		Log.d(LOG_TAG,self.current_rec_number)
		print(LOG_TAG,self.current_rec_number)
	end
--	Log.d(LOG_TAG,"send ("..#self.send_stack.."):")
	print(LOG_TAG,"send ("..#self.send_stack.."):")
	for i=1,#self.send_stack do
--		Log.d(LOG_TAG,"stack send number:"..self.send_stack[i].number)
		print(LOG_TAG,"stack send number:"..self.send_stack[i].number)
	end
end
...
--function to control messages to be sent and captured while also overcoming delivery failures
function Networking:update()
--	print("update")
	if self._gameFinish then return nil end
	if(self.myPlayer.id==nil)then -- jangan mulai bila id belum didapatkan, EN: don't start if id is not obtained
...
-- EN: send a message to the channel with that id
--be careful using this function, because if aMessage is a table and if the table is edited outside of Networking,
--then the message sent may be different because there was a change in the table while the message was being sent.
...
--onListen callback will be called if there is an incoming message to the channel with that id with the parameter
...
--				Log.d(LOG_TAG,"message type error")
				print(LOG_TAG,"message type error")
			end
		end
		end,
		errorback = function()
--			Log.d(LOG_TAG,"Networking : Listen, Connection Lost")
			print(LOG_TAG,"Networking : Listen, Connection Lost")
		end
	})
end

--stop mendengarkan pesan masuk pada channel dengan id tersebut
--stop listening to incoming messages on the channel with that id
...
--This function may be called more than once, so make sure it is safe to call it more than once.
function Networking:stopNetworking()
--	Log.d(LOG_TAG,"stop networking")
	print(LOG_TAG,"stop networking")
