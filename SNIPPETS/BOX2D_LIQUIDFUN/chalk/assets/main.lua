...
stage:addChild(Bitmap.new(Texture.new("gfx/board.jpg")))
chalkTextures = {}
table.insert(chalkTextures, Texture.new("gfx/chalks/chalk1.png", true))
table.insert(chalkTextures, Texture.new("gfx/chalks/chalk2.png", true))
table.insert(chalkTextures, Texture.new("gfx/chalks/chalk3.png", true))
table.insert(chalkTextures, Texture.new("gfx/chalks/chalk4.png", true))
table.insert(chalkTextures, Texture.new("gfx/chalks/chalk5.png", true))

sim = EasyBox2d.new{stage, debug=true}
--sim:createStageWalls{top=true, bottom=false, left=true, right=true, wallWidth=0.2}
sim:createStageWalls{top=true, bottom=true, left=true, right=true, wallWidth=0.5}
sim:start()
...
-- why a bullet here? demo test?
--local timer = Timer.new(2000, 1)
local timer = Timer.new(2000, 3)
timer:addEventListener(Event.TIMER_COMPLETE, function() 
	sim:addCircle { x=3, y=1, radius=0.5,
--			restitution=0.95, fillColor=0xcccccc, lineColor=0x000000, lineWidth=1,
			restitution=1, fillColor=0xcccccc, lineColor=0x000000, lineWidth=1,
			isBullet=true }
end)
timer:start()
