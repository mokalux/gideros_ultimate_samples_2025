...
function NdShape:oddRect(x,y,w,h)
   self:moveTo(x-w/2,0)
   self:quadraticCurveTo(x-w/2,y-h/2,0,y-h/2)
   self:quadraticCurveTo(x+w/2,y-h/2,x+w/2,0)
   self:quadraticCurveTo(x+w/2,y+h/2,0,y+h/2)
   self:quadraticCurveTo(x-w/2,y+h/2,x-w/2,0)
end
...
function NdShape:quadraticCurveTo(cpx, cpy, x, y, mu)
   local start = self.currentPoint and { x=self.currentPoint[1], y=self.currentPoint[2] }
   if start then
