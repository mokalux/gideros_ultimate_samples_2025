...
function cycleSoundMode()
	...
	lastOptionsBCaption = Caption.new(
		optionsB:getX(),
		optionsB:getY(),
		stageW * 0.75,
		stageH / 2,
		musicOn and "music ON" or "music off",
		soundOn and "sound ON" or "sound off"
	)
...
function toggleRunMode()
	...
	if maxLevel < 20 then
		lastRunModeCaption = Caption.new(
			runModeB:getX(),
			runModeB:getY() - 50,
			runModeB:getX(),
			runModeB:getY() - 160,
...
function intro()
	...
	if game.state == state.reseting or game.state == state.sharing then
		local alertDialog = AlertDialog.new(
			game.state == state.reseting and "All reset" or "Share Record",
			game.state == state.reseting and "Forget everything?" or "submit your best score on Reddit",
			"Cancel",
			game.state == state.reseting and "Reset now!" or "To Reddit!"
		)
		local signal = {}
		alertDialog:addEventListener(Event.COMPLETE, function(event)
			if event.buttonIndex == 1 then
				if game.state == state.reseting then
					...
					application:openUrl(
						"http://www.reddit.com/r/renabomb/submit?url=https%3A%2F%2Fplay.google.com%2Fstore%2Fapps%2Fdetails%3Fid%3Dcom.squishytoy.renabomb&title=got%20score%20" .. record .. "%20playing%20Renabomb%20Vengeance"
					)
				end
			end
...
function main()
    game:setState(
		((save.state == 'ingame') and
		state.ingame or (save.state == 'success') and state.success or state.intro)
	)
